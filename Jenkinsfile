        ...
        stage ('Docker Image Vulnerability Scan - TRIVY '){
            steps{
                script{
                    ...
                    curl -X POST   "http://18.216.55.201:8080/api/v2/engagements/" -H "Authorization: Token 494d5bcaf4cbadcb1f09df5f76f0a4389aaf5ad7"  -F "name=saelor-microservices-trivy-eg" -F "product=3" -F "target_start=2022-06-14" -F "target_end=2022-06-14"
                    #curl -X POST "http://18.216.55.201:8080/api/v2/reimport-scan/" -H  "accept: application/json" -H  "Content-Type: multipart/form-data" -H  "Authorization: Token 494d5bcaf4cbadcb1f09df5f76f0a4389aaf5ad7" -F "minimum_severity=Info" -F "active=true" -F "verified=true" -F "scan_type=Trivy Scan" -F "push_to_jira=false" -F "close_old_findings=true" -F "file=@/home/new_home/workspace/AWS-EKS-Microservices-Test/code/results.json" -F "product_name=saelor" -F "scan_date=2022-06-14" -F "engagement_name=saelor-microservices-trivy-eg" -F "engagement=8"
                    '''
