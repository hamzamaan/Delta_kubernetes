        ...
        stage('clone the kubernetes-app git repo '){
           ...
           stage('Code Analysis - sonarqube-saelor') {
            ...
            steps {
                ...
                curl -X POST "http://18.216.55.201:8080/api/v2/import-scan/" -H  "accept: application/json" -H  "Content-Type: multipart/form-data"  -H "Authorization: Token 494d5bcaf4cbadcb1f09df5f76f0a4389aaf5ad7" -F "active=true" -F "verified=true" -F "minimum_severity=Info" -F "active=true" -F "scan_type=Hadolint Dockerfile check" -F "file=@/home/new_home/workspace/AWS-EKS-Microservice/code/results.json" -F "product_name=saelor" -F "scan_date=2022-06-14" -F "engagement_name=saelor-microservices-lint-eg" -F "engagement=8"
                '''
